...
pub mod palette_map;
pub use palette_map::{map_file_to_palette, map_image_to_palette, ColorDistanceAlgorithm};
pub mod palettes;
pub use palettes::{load_palette_file, named_palette, named_palette_vec};
...
fn color_diff(c1: Vec3, c2: Vec3) -> f64 {
    c1.sub(c2).norm()
}
...
impl ColorEntry {
    fn condit_prob(&self, sp_color: Vec3, t: f64) -> f64 {
        self.probability * (-(color_diff(sp_color, self.color) / t)).exp()
    }
    fn perturb(&mut self, delta: Vec3) {
        self.color = self.color.add(delta);
    }
}
...
impl SuperPixel {
    ...
    fn normalize_probs(&mut self, palette: &[ColorEntry]) {
        ...
        if let Some((imax, _)) =
            self.p_c.iter().enumerate().max_by(|a, b| a.1.partial_cmp(b.1).unwrap_or(Ordering::Equal))
        {
            self.palette_color = palette[imax].color;
        }
        for v in &mut self.p_c {
            *v /= denom;
        }
    }
...
fn sp_refine(
    super_pixels: &Vec<Vec<Arc<RwLock<SuperPixel>>>>,
    in_image: &Vec<Vec<Vec3>>,
    num_threads: usize,
    w_in: usize,
    h_in: usize,
    w_out: usize,
    ...
    for t in 0..num_threads {
        ...
        let stride_x_c = stride_x;
        let stride_y_c = stride_y;
        ...
        let handle = thread::spawn(move || {
            ...
            for s in start..end {
                ...
                let rr = best_pair.0 as usize;
                let cc = best_pair.1 as usize;
                let p = in_c[x][y];
                let a = &mut acc[rr][cc];
                a.count += 1;
                a.sum_x += x as f64;
    ...
    for h in handles {
        thread_accums.push(h.join().unwrap_or_else(|_| vec![vec![LocalAccum::default(); h_out]; w_out]));
    }
    ...
    for r in 0..w_out {
        for c in 0..h_out {
            ...
            let mut sp = super_pixels[r][c].write();
            {
                ...
                acc.sum_x = total.sum_x;
                acc.sum_y = total.sum_y;
                acc.sum_l = total.sum_l;
                acc.sum_a = total.sum_a;
                acc.sum_b = total.sum_b;
            }
        }
    }
    ...
    for r in 0..w_out {
        for c in 0..h_out {
            ...
            if n == 0.0 {
                new_colors[r][c] = sp.sp_color;
            } else {
                avg = avg.scale(1.0 / n);
                new_colors[r][c] = sp.sp_color.scale(0.5).add(avg.scale(0.5));
            }
        }
    }
    for r in 0..w_out {
...
fn associate(
    super_pixels: &Vec<Vec<Arc<RwLock<SuperPixel>>>>,
    palette: &mut Vec<ColorEntry>,
    _clusters: &Vec<(usize, usize)>,
    t: f64,
) {
    for row in super_pixels.iter() {
        for sp in row.iter() {
            ...
            for k in 0..palette.len() {
                spw.p_c[k] = palette[k].condit_prob(spw.sp_color, t);
            }
            spw.normalize_probs(&palette);
        }
    }
    for k in 0..palette.len() {
        palette[k].probability = 0.0;
    }
    for row in super_pixels.iter() {
        for sp in row.iter() {
            let sp = sp.read();
...
fn expand(
    ...
    while i < *k && *k < k_max {
        ...
        if color_diff(c1, c2) > epsilon_cluster {
            *k += 1;
            ...
            palette[i0].probability = p0;
            palette[i1].probability = p1;
    ...
    if *k >= k_max {
        ...
        for idx in 0..*k {
            ...
            let c0 = palette[i0].color;
            let p0 = palette[i0].probability;
            let c1 = palette[i1].color;
            let p1 = palette[i1].probability;
...
pub fn process_dynamic(
    ...
    info!("Starting Pixel-convert (Rust) in-memory");
    ...
    let epsilon_palette = config.epsilon_palette;
    let epsilon_cluster = 0.25f64;
    let num_threads = config.num_threads;
    let m_full = w_in * h_in;
    let n = w_out * h_out;
    ...
    while t > t_final {
        ...
        if total_change < epsilon_palette {
            t *= alpha;
            if k < k_max {
                expand(&mut clusters, &mut palette, epsilon_cluster, &mut k, k_max, delta);
            }
        }
        if total_change < config.stag_eps {
            stagnant += 1;
        } else {
            stagnant = 0;
        }
        if stagnant >= config.stag_limit {
            info!("Early stop: converged (stagnation)");
            break;
        }
        if iterations >= 1010 {
        ...
        if let Some(start) = iter_start {
            info!("iter_time_ms={}", start.elapsed().as_millis());
        }
    }
...
pub fn process(params: Params) -> Result<()> {
    ...
    info!("Starting Pixel-convert (Rust)");
