...
in_rgb = [
    [[in_rgb[r][c][i] / 255 for i in range(3)] for c in range(h_in)]
    for r in range(w_in)
]
...
pca.fit(
    pca_form
)  # PCA = Principal Component Axis. This is the axis with the most variance.
...
num_threads = 6
is_debug = True  # Mutes debug print statements
...
class Color:
    ...
    def perturb(self):
        global delta
        self.color = (
            self.color[0] + delta[0],
            self.color[1] + delta[1],
            self.color[2] + delta[2],
        )
...
def palette_refine():
    ...
    for k in range(len(palette)):
        ...
        for row in super_pixels:
            for sp in row:
                for i in range(3):
                    new_color[i] += (sp.sp_color[i] * sp.p_c[k] * sp.p_s) / palette[
                        k
                    ].probability
...
def expand():
    ...
    for i in range(K):
        ...
        if color_diff(c1.color, c2.color) > epsilon_cluster:
            ...
            assert (
                abs(
                    palette[clusters[i][0]].probability
                    - palette[clusters[i][1]].probability
                )
                < epsilon_cluster
            )
            assert (
                abs(
                    palette[clusters[-1][0]].probability
                    - palette[clusters[-1][1]].probability
                )
                < epsilon_cluster
            )
...
while T > T_final:
    ...
    if iterations % 100 == 0 and iterations > 1:
        _idx = iterations // 100
        ...
        tmp_out_image_name = f"{out_image_name[:-4]}_{_idx}.png"
        output.save(tmp_out_image_name)
